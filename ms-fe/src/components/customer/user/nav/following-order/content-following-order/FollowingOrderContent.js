      ...
      try {
        ...
        if (response.data) {
          for (let i = 0; i < response.data.length; i++) {
            if (Number(id) !== Number(response.data[i].cartDetailResponse.productDetailId)) {
        ...
        <div className={styles.followingContent}>
          <Space direction="vertical" size={16} style={{ width: "100%" }}>
            <div style={{ borderBottom: "1px solid #ccc", padding: "5px 0" }}>
              ...
              >
                {status === '' && createdBy === 'CLIENT' ? ' Chờ giao hàng'
                  : status === 'Complete' ? 'Hoàn thành'
                    : status === 'Cancel' ? 'Đã hủy'
                      : status === '' && symbol === 'Shipping' && count === 2 ? 'Đã xác nhận'
                        : status === '' && count === 3 ? 'Đang giao hàng'
                          : 'Tất cả'}
              </span>
            </div>
            {
            ...
            }
            {
              (bills?.length > 0 && status === '') ? bills.map((timelines) => {
                return (
                  <>
                    ...
                    <Col span={24} style={{}}>
                      ...
                      >
                        <Button style={{ marginRight: 20 }} onClick={() => handleOpen(timelines[0].billId)} >
                          Chi tiết đơn hàng
                        </Button>
                ...
                return (
                  <Row style={{ padding: 10, borderBottom: '1px solid gray' }}>
                    <Col span={3}>
                      <Carousel style={{ maxWidth: "300px" }} autoplay>
                        {timeline.productImageResponses &&
                          timeline.productImageResponses.map(
                    ...
                    <Col span={3}>
                      ...
                      <Button type="primary" style={{ marginRight: "20px" }}
                        onClick={() => handleRePurchase(timeline.productDetailId)}
                      >
                        Mua lại
                      </Button>
                    </Col>
