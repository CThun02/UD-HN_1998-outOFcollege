...
const TimelineByBillCode = () => {
    ...
    const [timelineDíplay, setTimelineDisplay] = useState([])

    useEffect(() => {
        axios.get(`http://localhost:8080/api/client/getTimelineClientByBillCode/${billCode}`)
            .then((response) => {
                setLoading(true);
                setTimelines(response.data)
                var timelinesPush = [];
                for (let index = 0; index < response?.data?.lstTimeline.length; index++) {
            ...
            {loading ? (
                <div className={styles.width}>
                    <div className={styles.followingContent}>
                        ...
                        {timelines?.timelineCustomInfo ? <div style={{ display: "flex", justifyContent: "flex-end", marginTop: 50 }}>
                            <div style={{ width: "40%" }}>
                                <Row>
                                    <Col span={12} style={{ fontWeight: 500 }}>Tên khách hàng</Col>
                                    <Col span={12}>{timelines?.timelineCustomInfo?.fullName}</Col>
                                    <Col span={12} style={{ fontWeight: 500 }}>Số điện thoại</Col>
                                    <Col span={12}>{timelines?.timelineCustomInfo?.phoneNumber}</Col>
                                    ...
                                    <Col span={12}>
                                        ...
                                        ${timelines?.timelineCustomInfo?.ward.substring(0, timelines?.timelineCustomInfo?.ward.indexOf('|'))} 
                                        ${timelines?.timelineCustomInfo?.district.substring(0, timelines?.timelineCustomInfo?.district.indexOf('|'))} 
                                        ${timelines?.timelineCustomInfo?.city.substring(0, timelines?.timelineCustomInfo?.city.indexOf('|'))}`}
                                    </Col>
                                </Row>
                            </div>
                        </div> : <h3 style={{ display: "flex", justifyContent: "flex-end", marginTop: 50 }}> Khách lẻ</h3>}
                    </div>
                </div>
            ) : (
                <>Đang tải...</>
